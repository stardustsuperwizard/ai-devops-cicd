#!/usr/bin/env python3
"""Render the agent control plane dashboard for this project.
...
# the delicate-paths flag this dashboard prints, and the implementer-eligibility
...
# Applied by agent-02-implement.yml when its own project-validate.sh run does not
...
def render(model: dict, repo: str | None) -> str:
    ...
    if ready:
        ...
        if any(r.get("delicate") for r in ready):
            out += [
                "",
                "⚠️ means the task expects to touch a path flagged in "
                "`task_scope.DELICATE_*` — a format an automated path handles "
                "badly. Pick a strong model rather than a cheap tier.",
            ]
        if any(r.get("restricted") for r in ready):
            out += [
                ...
                "it, and `agent:implementer:*` is refused. Run it from a "
    ...
    if signoff:
        out += [
            ...
            "checks that only a human can do, then close it. Nothing closes a "
