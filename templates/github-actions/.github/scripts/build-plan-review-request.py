...
# A build cache alone is larger than everything a plan ever names.
SKIP_DIRS = frozenset(
    # TODO: add your build and cache directories.
    {".git", "__pycache__", ".import", "node_modules", ".venv", "dist", "build"}
)
...
# source method call is the noisy check 8 the brief forbids.
KNOWN_EXTENSIONS = (
    ".py", ".sh", ".yml", ".yaml", ".md", ".gd", ".tscn", ".tres",
    ".json", ".jsonl", ".toml", ".cfg", ".ini", ".txt",
...
def clean_token(raw: str) -> str:
    ...
    if token.startswith("res://"):               # an engine resource path
        token = token[len("res://"):]
...
def is_repository_shaped(token: str, kind: str, tree: Tree) -> bool:
    ...
    project configuration key and `OWNER/NAME` is an argument
...
def render_inventory(tree: Tree) -> str:
    lines = [
        ...
        " trees (`__pycache__`, `.import`, `node_modules`, `.venv`, build output)"
        " are excluded.",
        "",
    ]
