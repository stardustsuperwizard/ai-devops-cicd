...
- *Is this delicate?* Some file formats punish small mistakes -- binary or
  machine-serialized formats, generated assets, anything whose merge conflicts
  are unresolvable by hand. A task touching one wants a strong model rather
  than a cheap tier. The control plane flags it; nothing is blocked.
  CONFIGURE the three DELICATE_* tuples below for your project; leaving them
  empty simply disables the flag.
...
rejected at the remote:
    ...
    ! [remote rejected] HEAD -> agent-exec/168
      (refusing to allow a GitHub App to create or update workflow
       `.github/workflows/project-validation.yml` without `workflows`
       permission)

The failure is clean but expensive: it lands at the *push*, after a full
implementer session has already been paid for and produced a complete
implementation, and it is only diagnosable by reading the run log. In the
system this came from, one epic's decomposition hit it three times before the
check was added.
...
# agent-01-planner.yml's not-implementer-eligible run block and
...
# Paths where a weak or unknown model is a bad bet, but which the implementer
# can still push. Advisory only -- this flags, it never blocks.
#
# TODO: configure for your project. What belongs here is any path whose format
# an automated path handles badly: machine-serialized scene/asset formats,
# generated code, lockfiles, binary resources, anything whose merge conflicts
# a model cannot resolve. Empty tuples disable the flag cleanly.
#
# Examples from real projects:
#   game engine   (".tscn", ".tres"), ("project.godot",), ("addons/",)
#   web app       (".lock",), ("package-lock.json", "yarn.lock"), ("dist/",)
#   data platform (".avsc", ".proto"), (), ("migrations/",)
DELICATE_SUFFIXES: tuple[str, ...] = ()
DELICATE_NAMES: tuple[str, ...] = ()
DELICATE_PREFIXES: tuple[str, ...] = ()
...
def expected_paths(body: str) -> set[str]:
    ...
    - Any bare token on a bullet line. This is how the *planner* writes one.
      ...
      #168's section is literally `- .github/workflows/project-validation.yml`.
...
def evaluate(body: str) -> dict:
    ...
    return {
        "implementer_eligible": not restricted,
...
def main() -> int:
    ...
    source.add_argument(
        ...
        help=(
            ...
            "pushability instead of implementer eligibility."
        ),
    )
